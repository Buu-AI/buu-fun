...
import { ImageGlobe } from "./image-globe";
import { TextureManager } from "./texture-manger";
interface HeroLoadingWrapperProps {
  children?: ReactNode;
  imageUrls: string[];
  loadingConfig?: {
...
export const HeroLoadingWrapper = ({
  children,
  imageUrls,
  loadingConfig = DEFAULT_LOADING_CONFIG,
  ...
  useEffect(() => {
    ...
    textureManager.preloadTextures(imageUrls).catch(console.error);
  }, [imageUrls]);
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      progressRef.current +=
        Math.random() *
        (loadingConfig.incrementRange || DEFAULT_LOADING_CONFIG.incrementRange);
    }, loadingConfig.interval || DEFAULT_LOADING_CONFIG.interval);
    ...
    <div className="w-full h-full relative">
      <div className="w-full h-screen relative">
        ...
        >
          ...
          <ImageGlobe finishedLoading={finishedLoading} imageUrls={imageUrls} />
          <EffectComposer>
            <Bloom />
          </EffectComposer>
        </Canvas>
