...
import { FeeManager as FeeManagerProgramType } from "./descriptor/fee_manager";
import FeeManagerIDL from "./descriptor/idl/fee_manager.json" with { type: "json" };
import RewardPoolIDL from "./descriptor/idl/reward_pool.json" with { type: "json" };
import StakePoolIDL from "./descriptor/idl/stake_pool.json" with { type: "json" };
import { RewardPool as RewardPoolProgramType } from "./descriptor/reward_pool";
import { StakePool as StakePoolProgramType } from "./descriptor/stake_pool";
import {
  deriveConfigPDA,
  deriveFeeValuePDA,
  deriveRewardPoolPDA,
...
export class SolanaStakingClient {
  ...
  constructor({
    ...
    const stakePoolIdl = {
      ...StakePoolIDL,
      address:
        programIds?.stakePool ??
        STAKE_POOL_PROGRAM_ID[cluster] ??
        StakePoolIDL.address,
    } as StakePoolProgramType;
    const rewardPoolIdl = {
      ...RewardPoolIDL,
      address:
        programIds?.rewardPool ??
        REWARD_POOL_PROGRAM_ID[cluster] ??
        RewardPoolIDL.address,
  ...
  getCommitment(): Commitment | undefined {
    return typeof this.commitment == "string"
      ? this.commitment
      : this.commitment.commitment;
  }
  ...
  async prepareCreateStakePoolInstructions(
    ...
    const stakePoolPDA = deriveStakePoolPDA(
  ...
  async prepareFundPoolInstructions(
    ...
    return {
      ixs: treasuryATA ? treasuryATA.concat([instruction]) : [instruction],
    };
  }
  ...
  decode<
    ...
    DerivedIdl extends Idl = DecodingProgram extends Program<infer IDLType>
      ? IDLType
      : never,
    AccountName extends
      keyof IdlAccounts<DerivedIdl> = keyof IdlAccounts<DerivedIdl>,
